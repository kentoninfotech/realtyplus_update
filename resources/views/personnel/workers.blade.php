...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Workers <span class="text-muted fs-6">({{ $workers->total() }})</span></h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle" id="products">
            ...
            <tbody>
                ...
                <tr>
                    <!-- <td><input type="checkbox" name="select_worker[]" value="{{-- $worker->id --}}"></td> -->
                    <td class="d-flex align-items-center">
                        <img src="{{ (isset($worker->personnel->picture) && $worker->personnel->picture !== null) ? asset('public/personnel/pictures/' .$worker->personnel->picture) : 'https://ui-avatars.com/api/?name=' . urlencode($worker->name) }}" class="rounded-circle me-2" width="40" height="40" alt="{{ $worker->name }}">
                    ...
                    <td>{{ $worker->personnel->designation ?? 'N/A' }}
                        <div class="text-muted small">{{ $worker->personnel->department ?? 'N/A' }}</div>
                    </td>
                    <td>{{ $worker->personnel->staff_id ?? '' }}</td>
                    <td>
                        @foreach($worker->getRoleNames() ?? [] as $role)
                            <span class="badge
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu text-center">
                                @can('view user')
                                    <a class="dropdown-item" href="{{ route('show.personnel', $worker->id) }}"><i class="fa fa-eye"></i> View</a>
                                @endcan
                                @can('edit user')
                                    <a class="dropdown-item" href="{{ route('edit.personnel', $worker->id) }}"><i class="fa fa-edit"></i> Edit</a>
                                @endcan
                                @hasanyrole('System Admin|Super Admin') 
                                    <a class="dropdown-item" href="{{ route('user.role.edit', $worker) }}">
                                ...
                                @can('delete user')
                                    ...
                                    <form class="d-inline" action="{{ route('delete.personnel', $worker->id) }}" method="post">
                                        @csrf
                 ...
                 @else
                <tr>
                    <td colspan="7" class="text-center text-muted">No worker found.</td>
                </tr>
                @endif
            </tbody>
        </table>
